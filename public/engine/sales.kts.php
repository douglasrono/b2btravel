...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($cartDetails as $cartItem) {
            $itemName = $cartItem['name']; // Fix variable name here
            $itemAttribute = $cartItem['attribute']; // Fix variable name here
            $itemSize = $cartItem['size']; // Fix variable name here
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $subtotal = $quantityToDeduct * $cartItem['price']; // Use $cartItem instead of $item
